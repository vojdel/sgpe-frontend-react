...
const Tabla = ({ nombres, datas, changeRegistro, changeId }) => {
  const handleDelete = (event, id) => {
    ...
    destroy(id, 'municipio').then(data => {
      console.log(data)
              ...
              datas.map((data, index) => {
                return (
                  <tr key={index}>
                    ...
                    <td>
                      <div className="text-xs font-weight-bold mb-0 text-center">{data.states}</div>
                    </td>
                    <td>
                      <div className="text-xs font-weight-bold mb-0 text-center">{data.municipalitys}</div>
                    </td>
