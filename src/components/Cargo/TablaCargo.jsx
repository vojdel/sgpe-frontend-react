...
const TablaCargo = ({ nombres, datas, changeRegistro, changeId }) => {
  const handleDelete = (event, id) => {
    event.preventDefault()
    destroy(id, 'cargo').then(data => {
      console.log(data)
      getAll('cargo').then(response => {
              ...
              datas.map((data, index) => {
                return (
                  <tr key={index}>
                    ...
                    <td>
                      <div className="text-xs font-weight-bold mb-0">{data.cargos}</div>
                    </td>
